  ...
  module HasSmsVerification
    ...
    module ClassMethods
      ...
      module InstanceMethods
        ...
        #
        def verify!(code)
          # if send(self.class.sms_verification_column).nil?
            result = verify code
            if result
        ...
        def aes_check
          Dji::Security.aes_decrypt send(self.class.sms_mobile_column), Settings.sign_key.real_name_aes_key
        end
        ...
        # 发送短信至手机
        #
        def deliver(template = :default_template, variables = { code: SmartSMS::VerificationCode.random })
          mobile = Dji::Security.aes_decrypt send(self.class.sms_mobile_column), Settings.sign_key.real_name_aes_key
          template_text = SmartSMS.config.templates[template]

          placeholder = /%{[^}]+}/
          values = template_text.scan(placeholder).map do |variable|
            variable = variable[2..-2]
            variables[variable.to_sym].to_s
          end

          text = template_text.split(placeholder).each_with_index.map do |segment, index|
            values[index].nil? ? segment : segment.concat(values[index])
          end.join

          SmartSMS.deliver mobile, text
          sms = SmartSMS::FakeSMS.build_fake_sms mobile, text
          save_or_return_message sms, variables[:code]
        end

        def deliver_fake_sms(template = :default_template, variables = { code: SmartSMS::VerificationCode.random })
          mobile = Dji::Security.aes_decrypt send(self.class.sms_mobile_column), Settings.sign_key.real_name_aes_key
          template_text = SmartSMS.config.templates[template]
